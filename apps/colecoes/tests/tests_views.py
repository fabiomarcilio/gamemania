...
# python manage.py test apps.colecoes.tests.tests_views


class ColecaoViewTestCase(TestCase):
    # Verifica se o status de retorno da view é 200
    def test_status_code_200(self):
        # client é utilizado para simular uma requisição
        response = self.client.get(reverse('colecao:form'))
        self.assertEqual(response.status_code, 200)
    # Testa se o template é chamado corretamente.

    def test_template_usado(self):
        response = self.client.get(reverse('colecao:form'))
        self.assertTemplateUsed(response, 'colecoes/form.html')
